...
def update_label(l, text, var):
    ...
    #f0_read_file.config(text="Файл для шифрования - {0}".format(read_file))
    #f0_par_file.config(text="Файл параметров - {0}".format(par_file))
    #f0_save_file.config(text="Конечный файл - {0}".format(save_file))
    
    #f1_read_file.config(text="Файл для шифрования - {0}".format(read_file))
    #f1_par_file.config(text="Файл параметров - {0}".format(par_file))
    #f1_save_file.config(text="Конечный файл - {0}".format(save_file))

    #fk_private_save.config(text="Приватный ключ - {0}".format(RSA_private))
    #fk_public_save1.config(text="Публичный ключ - {0}".format(RSA_public))
    #fk_public_save2.config(text="Публичный ключ - {0}".format(RSA_public))
...
def create_top_info():
 ...
 программа,\nнаписанная на языке Python 3 с использованием Pycryptodome и Tkinter", font="Arial 11")
    fi_header.pack()
    fi_name.pack()
    fi_lbl.pack()
    return ti
